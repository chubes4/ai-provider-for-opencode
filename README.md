...
This is intentionally small: it uses the AI Client's chat-completions implementation, discovers local OpenCode state when available, and exposes only models returned by OpenCode or declared in local OpenCode config/state. More models can be added with the `ai_provider_for_opencode_model_metadata` filter.
